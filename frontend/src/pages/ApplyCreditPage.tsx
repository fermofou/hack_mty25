...
export default function ApplyCreditPage() {
  ...
  const buildConversationContext = (msgs: Message[]) => {
    return msgs
      ...
      .map((m) =>
        m.role === 'user' ? `cliente: ${m.content}` : `AI: ${m.content}`
      )
      .join('\n');
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (data.response_type === 'text') {
        ...
        data.response_type === 'credit' &&
        data.creditOffers?.creditOffers
      ) {
        setMessages((prev) => [
          ...prev,
          {
            role: 'assistant',
            content: `Te recomiendo estas opciones de crédito para: ${data.object_in_response}`,
          },
        ...
        setMessages((prev) => [
          ...
          {
            role: 'assistant',
            content: 'No se pudo procesar la respuesta de la IA.',
          },
        ]);
      }
    } catch (err) {
      ...
      setMessages((prev) => [
        ...
        {
          role: 'assistant',
          content: 'Ocurrió un error al contactar a la IA.',
        },
      ]);
    } finally {
      setIsLoading(false);
      ...
      <main className='container mx-auto px-4 py-8 max-w-4xl'>
        ...
        <div className='mb-8'>
          ...
          <p className='text-muted-foreground ml-13'>
            Completa la información con nuestro asistente virtual
          </p>
        </div>
                      ...
                      <div className='text-sm leading-relaxed markdown-content'>
                        <ReactMarkdown
                          components={{
                            ...
                            ol: ({ children }) => (
                              <ol className='mb-2 pl-4 list-decimal'>
                                {children}
                              </ol>
                            ),
                            li: ({ children }) => (
                            ...
                            h1: ({ children }) => (
                              <h1 className='text-lg font-semibold mb-2 mt-2 first:mt-0'>
                                {children}
                              </h1>
                            ),
                            h2: ({ children }) => (
