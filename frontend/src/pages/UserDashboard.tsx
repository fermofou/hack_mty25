...
export default function UserDashboard() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.detail || 'Error al registrar transacción'
      );
    } finally {
      setSubmitting(false);
    }
      ...
      <main className='container mx-auto px-4 py-8 max-w-[1200px]'>
        ...
        {!loadingCredits && preapprovedCredits.length > 0 && (
          <PreapprovedCreditCard
            ...
            onApply={() => {
              navigate(
                `/user/credits/details/${preapprovedCredits[0].id_cred}`
              );
            }}
            onDismiss={async () => {
              setPreapprovedCredits([]);
              await api.patch(
                `clientes/${user.id}/creditos/${preapprovedCredits[0].id_cred}/negar`
              );
            }}
          />
        )}
        ...
        <div className='flex items-center justify-between mt-12 mb-2'>
          ...
          <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
            <div className='px-2 pt-6 pb-2'>
              <h2 className='text-2xl font-bold text-[#23272F] mb-8 text-center'>
                Registrar transacción
              </h2>
              <form
              ...
              >
                <div className='flex flex-col gap-6'>
                  <div className='flex flex-col gap-2'>
                    <label className='text-[16px] font-bold text-[#23272F] mb-1'>
                      Monto
                    </label>
                    <Input
                      name='monto'
                  ...
                  <div className='flex flex-col gap-2'>
                    ...
                    <div className='relative'>
                      ...
                      <span className='pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-[#A0A4A8] text-lg'>
                        ▼
                      </span>
                    </div>
                  </div>
                ...
                <div className='flex justify-end gap-3 pt-6 mt-2 border-t border-[#F0F1F3]'>
                  <Button
                    ...
                    disabled={submitting}
                    className='min-w-[120px] text-[16px] font-semibold border border-[#A0A4A8] text-[#23272F] bg-white hover:bg-gray-100 rounded-lg py-2'
                  >
                    Cancelar
                  </Button>
                  <Button
                    ...
                    variant='primary'
                    disabled={submitting}
                    className='min-w-[120px] text-[16px] font-semibold bg-[#EB0029] hover:bg-[#c90022] rounded-lg py-2'
                  >
