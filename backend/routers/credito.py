...
async def get_preapproved_credit_endpoint(
    ...
    # Retrieve ALL pre-approved credits (estado="APROBADO") for this user
    statement = select(Credito).where(
        Credito.cliente_id == cliente_id, Credito.estado == "APROBADO"
    )
    result = await session.execute(statement)
    all_preapproved = result.scalars().all()

    # Convert database records to CreditOffer format
    all_offers = []
    for credito in all_preapproved:
        # Get the item if it exists
        if credito.item_id:
            item = await session.get(Item, credito.item_id)
            if item:
                ...
                product_data = ProductData(
                    nombre=credito.descripcion or "Product",
                    link="",
                    img_link="",
                    precio=credito.prestamo,
                    categoria=credito.categoria or "",
                )
        else:
            # Fallback if no item linked
            product_data = ProductData(
                nombre=credito.descripcion or "Product",
                link="",
                img_link="",
        ...
        all_offers.append(credit_offer)

    return CreditOffers(creditOffers=all_offers)
